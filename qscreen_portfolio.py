...
def _pct(x):
    return "—" if x is None else f"{(x * 100 if abs(x) <= 1.5 else x):.1f}%"
...
def render_html(board: dict) -> str:
    ...
    for i, r in enumerate(board["rows"], 1):
        ...
        val = (f"{r['currency'] or ''} {_num(r['per_share'])}/sh" if r.get("per_share")
               else (f"{r['currency'] or ''} {_num(r['equity_value'])}" if r.get("equity_value") else "—"))
        ...
        P.append(f"<tr><td>{i}</td><td>{r['symbol']}</td><td>{E(str(r['name']))}</td>"
                 f"<td class='muted'>{r['archetype'].replace('_', ' ')}</td><td>{r['year'] or '—'}</td>"
